...
def train_classifier(ref_fasta, ref_taxonomy_tsv, k=7):
    """Train a Naive Bayes k-mer classifier. Returns (vectorizer, classifier)."""
    ...
    ref_kmers = [kmerize(r.seq, k) for r in ref_records]
...
def classify_sequences(query_fasta, vectorizer, clf, k=7):
    """Classify query sequences. Returns a DataFrame."""
    query_records = list(SeqIO.parse(query_fasta, "fasta"))
    if not query_records:
        raise ValueError(f"No sequences found in {query_fasta}")

    query_kmers = [kmerize(r.seq, k) for r in query_records]
...
if __name__ == "__main__":
    ...
    vectorizer, clf = train_classifier(ref_fasta, ref_taxonomy_tsv)
    results = classify_sequences(query_fasta, vectorizer, clf)
